...
def gen_coeff_signal2d(f, PS, nc):
    ...
    #''' Compute nc random vectors in an annular (radius is between (min(f), max(f)) '''
    f1=min(f)
    f2=max(f)
    logf=numpy.log10(f)
...
def rotationmat3D(theta, axis):
    ...
    The rotation matrix correspond to a rotation of angle theta with respect to axis axis. \n
...
def cart2spher(x, y, z):
    ...
    if x<0:
        lon=numpy.arctan(y/x)%(2*math.pi) +  max((x < 0) - (x > 0),0)*math.pi #+ max(-sign(x),0)*math.pi
    else: 
