...
# Compute swath grid if pass is in the subdomain
        if nind>5:
            mod_tools.update_progress(float(ipass+1)/float(numpy.shape(passtime)[0]), 'selected pass: '+ str(ipass+1), None)
# Initialize SWOT grid, grid variables and Satellite direction and Location
