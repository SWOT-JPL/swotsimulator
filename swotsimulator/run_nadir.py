...
between model files and thus if several files are used in the
SSH interpolation, some discontinuities may be visible. \n
...
def run_nadir(file_param):
    ...
    if p.modelbox:
        ...
        if modelbox[0] < 0:
            modelbox[0] = modelbox[0] + 360
        if modelbox[1] < 0:
            modelbox[1] = modelbox[1] + 360
    else:
        try:
    ...
    if p.file_input:
        ...
        model_index = numpy.where(((modelbox[0] - 1) <= model_data.vlon)
    ...
    if p.file_coeff:
        if os.path.isfile(p.file_coeff[:-3] + '_nadir.nc') and \
                (not p.makesgrid):
    ...
    for filesat in p.filesat:
        ...
        if p.makesgrid:
            ...
            al_cycle = 0
            timeshift = 0
            orb.load_orb(cycle=cycle, x_al=x_al, al_cycle=al_cycle,
                         timeshift=timeshift)
        cycle = orb.cycle
        x_al = orb.x_al
        al_cycle = orb.al_cycle
        timeshift = orb.timeshift
        ...
        if numpy.min(orb.lon) < 1. and numpy.max(orb.lon) > 359.:
            ...
            if p.file_input:
                model_data.vlon[numpy.where(model_data.vlon > 180.)] = model_data.vlon[numpy.where(model_data.vlon > 180.)] - 360
            if modelbox[0] > 180.:
                modelbox[0] = modelbox[0] - 360
            if modelbox[1] > 180.:
                modelbox[1] = modelbox[1] - 360
        if p.file_input:
            model_index = numpy.where((numpy.min(orb.lon) <= model_data.vlon)
                                      & (model_data.vlon <= numpy.max(orb.lon))
                                      & (numpy.min(orb.lat) <= model_data.vlat)
                                      & (model_data.vlat <= numpy.max(orb.lat)))
        ...
        for cycle in range(0, ncycle+1):
            ...
            SSH_true = numpy.zeros((numpy.shape(orb.x_al)[0]))
            ...
            # Look for satellite data that are between step-p.timesetp/2
            # and step+p.step/2
            if p.file_input:
                index = numpy.where(((stime[-1]-timeshift) >= (modeltime-p.timestep/2.)) & ((stime[0]-timeshift) < (modeltime+p.timestep/2.)))
                # At each step, look for the corresponding time in the
                # satellite data
                for ifile in index[0]:
                    ntot = 1
                    mod_tools.update_progress(float(istep)/float(p.nstep*ntot
                                              * numpy.shape(p.filesat)[0]),
                                              'Sat: ' + filesat[istring:],
                                              'model file: ' + list_file[ifile]
                                              + ', cycle:' + str(cycle+1))
                    ...
                    if numpy.shape(index)[1] > 0:
                        ...
                        if p.interpolation == 'nearest':
                            if modelbox[0] > modelbox[1]:
                                SSH_true[numpy.where(((orb.lon < modelbox[0])
                                         & (orb.lon > modelbox[1]))
                                         | (orb.lat < modelbox[2])
                                         | (orb.lat > modelbox[3]))] = numpy.nan
                            else:
                                SSH_true[numpy.where((orb.lon < modelbox[0])
                                         | (orb.lon > modelbox[1])
                                         | (orb.lat < modelbox[2])
                ...
                istep += 1
                mod_tools.update_progress(float(istep)/float(rcycle
                                          * numpy.shape(p.filesat)[0]),
                                          'Sat: ' + filesat[istring:],
                                          'no model file provide ' + ', cycle:'
                                          + str(cycle+1))

            SSH_true[SSH_true == 0] = numpy.nan
            errnad.make_error(orb, cycle, SSH_true, p)  # , ind[0])
            if p.wet_tropo:
                errnad.SSH = SSH_true + errnad.nadir + errnad.wet_tropo1
            else:
                errnad.SSH = SSH_true + errnad.nadir
            # Save outputs in a netcdf file
            if vindice.any() or not p.file_input:
                file_output = (p.file_output + '_c' + str(cycle+1).zfill(2)
                               + '.nc')
                Output = rw_data.Sat_nadir(file=file_output,
                                           lon=(orb.lon+360) % 360,
                                           lat=orb.lat, time=stime,
                                           x_al=orb.x_al, cycle=orb.cycle)
                Output.write_data(SSH_model=SSH_true, index=vindice,
                                  nadir_err=errnad.nadir, SSH_obs=errnad.SSH,
                                  pd_err_1b=errnad.wet_tropo1, pd=errnad.wt)
        del stime
        if p.file_input:
            del index
